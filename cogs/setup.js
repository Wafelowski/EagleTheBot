    ...
    if (args[0] == "info") {
        ...
        serverDB.all('SELECT * FROM roles WHERE role = ?',"admin", function(err, rows) {
            ...
            if (rows === undefined) {
                ...
                //msg.reply(`modsID - ${adminsID.join(', ')}`)
            }
        });
        // let query2 = `SELECT * FROM roles WHERE role = ?`;
        // serverDB.get(query2, "admin", (err, row) => {
        //     if (err) {
        //         adminID = "Wystąpił błąd - Administrator"
        //         console.log(err);
        //         return;
        //     }
        //     if (row === undefined) {
        //         adminID = "Brak: Administrator"
        //         return;
        //     } else {
        //         adminID = row.roleid;
        //         //msg.reply(`modsID - ${modID}`)
        //     }
        // });
        serverDB.close()
        // const embed = new Discord.MessageEmbed()
        //     .setAuthor(`Setup - ${msg.guild.name}`)
        //     .setDescription(`Administratorzy - ${adminsID.join(', ')}. \nModeratorzy - ${mods.join(', ')}.`)
        //     .setColor("#ff0000")
